...
- `pinStore` — app lock state (`pin` | `biometric`), drives the `setup-pin` / `unlock` routing. **Mandatory on native** (no lock configured → forced to `setup-pin`); **optional on web/PWA** — enabled from Settings › Security (PIN, or Face ID / Touch ID / Hello / security key via `webauthnService`, `lockMethod: 'webauthn'`), and re-armed after 5 min in the background by `useWebAutoLock`. WebAuthn here is a **local lock** (client-side challenge, no server verification) exactly like native biometrics — passwordless passkey *login* would need OctoberCMS register/verify endpoints. The PIN is SHA-256 salted before storage wherever WebCrypto exists (web); on native it stays raw inside the OS-encrypted secure store. The login password is never persisted on web.
