...
import { RefreshableScrollView } from '../components/Refreshable';
...
export default function WalletsScreen() {
  ...
  const loadWallets = useCallback(
    () => walletService.getSavedWallets()
      ...
      .catch(() => setLoadError(t('account.walletsLoadError'))),
    [t]
  );
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try { await loadWallets(); } finally { setRefreshing(false); }
  }, [loadWallets]);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.formModalOverlay}>
        ...
        <View style={styles.formModalContainer}>
          <View style={styles.formModalHeader}>
            ...
            <CloseButton onPress={resetWalletForm} size={16} color={Colors.textMuted} />
          </View>
